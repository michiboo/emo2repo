	...
	private void sendRequestSetup() throws IllegalStateException, SocketException, IOException {
		for (int i=0;i<2;i++) {
			...
			if (stream != null) {
				...
				String request = "SETUP rtsp://"+mParameters.host+":"+mParameters.port+mParameters.path+"/stream="+0+" RTSP/1.0\r\n" +
...
//				System.out.println(request);
				...
				System.out.println(response.headers);
				Matcher m;
				System.out.println(response.headers.get("session"));
				if (response.headers.containsKey("session")) {
					try {
